...
const DistributionFunctions = {
  'BETA.DIST': (x, alpha, beta, cumulative, a, b) => {
    ...
    if (alpha <= 0 || beta <= 0 || x < a || x > b || a === b) throw FormulaError.NUM;
  ...
  'BETA.INV': (probability, alpha, beta, a, b) => {
    ...
    a = H.accept(a, Types.NUMBER, 0);
    b = H.accept(b, Types.NUMBER, 1);
    if (alpha <= 0 || beta <= 0 || probability <= 0 || probability > 1) throw FormulaError.NUM;
  ...
  'BINOM.DIST': (numberS, trials, probabilityS, cumulative) => {
    ...
    cumulative = H.accept(cumulative, Types.BOOLEAN);
    if (trials < 0 || probabilityS < 0 || probabilityS > 1 || numberS < 0 || numberS > trials) throw FormulaError.NUM;
  ...
  'BINOM.INV': (trials, probabilityS, alpha) => {
    ...
    probabilityS = H.accept(probabilityS, Types.NUMBER);
    alpha = H.accept(alpha, Types.NUMBER);
    if (trials < 0 || probabilityS < 0 || probabilityS > 1 || alpha < 0 || alpha > 1) throw FormulaError.NUM;
  ...
  'CHISQ.DIST': (x, degFreedom, cumulative) => {
    ...
    if (x < 0 || degFreedom < 1 || degFreedom > 10 ** 10) throw FormulaError.NUM;
  ...
  'CHISQ.INV': (probability, degFreedom) => {
    ...
    degFreedom = H.accept(degFreedom, Types.NUMBER);
    degFreedom = Math.trunc(degFreedom);
    if (probability < 0 || probability > 1 || degFreedom < 1 || degFreedom > 10 ** 10) throw FormulaError.NUM;
  ...
  'CHISQ.TEST': (actualRange, expectedRange) => {
    ...
    for (let i = 0; i < row; i++) {
      for (let j = 0; j < col; j++) {
        if (typeof actual[i][j] !== 'number' || typeof expected[i][j] !== 'number') continue;
  ...
  'CONFIDENCE.NORM': (alpha, std, size) => {
    ...
    if (alpha <= 0 || alpha >= 1 || std <= 0 || size < 1) throw FormulaError.NUM;
  ...
  CORREL: (array1, array2) => {
    ...
    for (let i = 0; i < array1.length; i++) {
      if (typeof array1[i] !== 'number' || typeof array2[i] !== 'number') continue;
  ...
  FORECAST: (x, knownYs, knownXs) => {
    ...
    for (let i = 0; i < knownYs.length; i++) {
      if (typeof knownYs[i] !== 'number' || typeof knownXs[i] !== 'number') continue;
