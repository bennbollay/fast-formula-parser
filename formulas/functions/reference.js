...
const ReferenceFunctions = {
  ADDRESS: (rowNumber, columnNumber, absNum, a1, sheetText) => {
    ...
    if (rowNumber < 1 || columnNumber < 1 || absNum < 1 || absNum > 4) throw FormulaError.VALUE;
  ...
  COLUMN: (context, obj) => {
    if (obj == null) {
      ...
      else throw Error('FormulaParser.parse is called without position parameter.');
    } else {
  ...
  INDEX: (context, ranges, rowNum, colNum, areaNum) => {
    ...
    if (rowNum !== 0 && colNum !== 0) {
      ...
      // array constant
        ...
        if (range.length < rowNum || range[0].length < colNum) throw FormulaError.REF;
