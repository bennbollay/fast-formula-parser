...
class FormulaParser {
   ...
   * @param offset - [number, number] - Position in the command string.
   * @return {*}
   */
  _callFunction(name, args, offset) {
    ...
    if (this.functions[name]) {
      ...
      try {
        ...
        this.parser.tokVector[this.parser.currIdx].bracket = JSON.parse(JSON.stringify(offset));
      } catch (e) {
  ...
  async callFunctionAsync(name, args, offset) {
    const awaitedArgs = [];
    for (const arg of args) {
      awaitedArgs.push(await arg);
    }
    const res = await this._callFunction(name, awaitedArgs, offset);
    return FormulaHelpers.checkFunctionResult(res);
  }

  callFunction(name, args, offset) {
    if (this.async) {
      return this.callFunctionAsync(name, args, offset);
    } else {
      const res = this._callFunction(name, args, offset);
      return FormulaHelpers.checkFunctionResult(res);
    }
  }
