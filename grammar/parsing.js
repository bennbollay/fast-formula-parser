...
class Parsing extends EmbeddedActionsParser {
  ...
  constructor(context, utils) {
    ...
    $.RULE('plusMinusOp', () => $.OR([{ ALT: () => $.CONSUME(PlusOp).image }, { ALT: () => $.CONSUME(MinOp).image }]));
    ...
    $.RULE('formulaWithUnaryOp', () => {
      ...
      const prefixes = [];
      $.MANY(() => {
        const op = $.OR([{ ALT: () => $.CONSUME(PlusOp).image }, { ALT: () => $.CONSUME(MinOp).image }]);
        prefixes.push(op);
      });
    ...
    $.RULE('formulaWithRange', () => {
      ...
      if (refs.length > 1) return $.ACTION(() => $.ACTION(() => this.utils.applyRange(refs)));
      return ref1;
    });
    ...
    $.RULE('paren', () => {
      ...
      if (refs.length > 1) result = $.ACTION(() => this.utils.applyUnion(refs));
      else result = refs[0];
    ...
    $.RULE('constantArray', () => {
      ...
      $.MANY(() => {
        const sep = $.OR([{ ALT: () => $.CONSUME(Comma).image }, { ALT: () => $.CONSUME(Semicolon).image }]);
      ...
      $.OR([
        {
          ALT: () => {
            ...
            if (prefix) return $.ACTION(() => this.utils.applyPrefix([prefix], number));
            return number;
          },
        },
        {
    ...
    $.RULE('functionCall', () => {
      ...
      const functionName = functionToken.image.slice(0, -1);
      // console.log('functionName', functionName);
      const args = $.SUBRULE($.arguments);
      const closeToken = $.CONSUME(CloseParen);
      // dependency parser won't call function.
      return $.ACTION(() =>
        context.callFunction(functionName, args, [functionToken.startOffset, closeToken.endOffset])
      );
    });
      ...
      $.OR([
...

        {
          ...
          ALT: () => {
            ...
            $.ACTION(() => {
              if (this.utils.isFormulaError(referenceItem)) return referenceItem;
